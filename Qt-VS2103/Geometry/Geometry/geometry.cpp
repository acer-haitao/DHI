...
Geometry::Geometry()//第一个为类，第二个类中的函数
{
    ...
    updateLabel();//跟新各个函数的值：
}

void
Geometry::updateLabel()
{
    QString temp;//以下程序为获取各个坐标函数的值并显示在相应的label上
    QString str_x;
    ...
    QString r;//此处有问题rect().width()和rect().height()函数显示为0（error）
...
void 
Geometry::moveEvent(QMoveEvent *)//里面的参数为窗体移动事件
{
    updateLabel();
}
